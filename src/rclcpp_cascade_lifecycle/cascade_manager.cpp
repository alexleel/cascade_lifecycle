...
bool CascadeManager::manager_configure(const std::string node_list_name)
{
    ...
    if (manager_type != SINGLE_MANAGER) {
        ...
        if (node_name_list.size() != 0) {
            for (auto & node_name : node_name_list) {
                if (node_map_.find(node_name) == node_map_.end()) {
                    node_map_.insert(std::pair<std::string, uint8_t>(node_name, lifecycle_msgs::msg::State::PRIMARY_STATE_UNKNOWN));
                    message_info(std::string("Add ") + node_name + std::string(" to node chain list of ") + this->get_name());
                }
            }
        ...
        node_states_ = this->create_subscription<cascade_lifecycle_msgs::msg::State>("cascade_lifecycle_states", 
            rclcpp::QoS(100),
...
bool CascadeManager::manager_activate()
{
    ...
    if (manager_type != SINGLE_MANAGER) {
        ...
        thread_flag_ = true;
        sub_node_checking = std::make_unique<std::thread>(&CascadeManager::node_status_checking, this, IS_ACTIVE);
        rtn_ = true;
  }
...
void CascadeManager::node_status_checking(const State_Req req_type)
{
        ...
        (this->get_clock()->now() - start_time <= std::chrono::seconds(timeout_manager_)))
    {
        ...
        for (auto & activation : activations) {
            if (node_map_[activation] == req_type &&
                ...
                message_info(std::string("Node [") + activation + std::string("] is ") + state_map_[req_type]);
    ...
    if (activations.size() != 0) {
        for (auto & activation : activations) {
            if (node_map_[activation] != req_type) {
                unchanged_count_ += 1;
                message_error( std::string("After ") + std::to_string(timeout_count) + std::string(" seconds. Node ") 
                    + activation + std::string(" is still not ") + state_map_[req_type]);
            }
        }
    }
    if (unchanged_count_ == 0) {
        chainnodes_state_ = (req_type == IS_ACTIVE) ? ALL_ACTIVE : ALL_DEACTIVE;
        message_info( std::string("All node/nodes is/are ") + state_map_[req_type]);
    } else {
